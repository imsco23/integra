<div class="card shadow-sm rounded-0">
    <div class="card-header">
        <div class="d-flex justify-content-between align-items-center flex-wrap">
            <h5 class="mb-2 mb-md-0">List of Suppliers</h5>
            <a href="javascript:void(0)" id="new_supplier" class="btn btn-sm btn-primary">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-bordered table-striped table-sm mb-0" id="supplier_table">
                ...
                <tbody>
                    <?php 
                    $i = 1;
                    $sup_qry = $conn->query("SELECT * FROM `supplier_list` WHERE delete_flag = 0 ORDER BY `name` ASC");
                    while($row = $sup_qry->fetch_assoc()):
                    ?>
                    <tr>
                        <td class="text-center"><?php echo $i++; ?></td>
                        <td><?php echo $row['name'] ?></td>
                        <td><?php echo $row['supplier_contact'] ?: "N/A" ?></td>
                        ...
                        <td class="text-center">
                            <a href="javascript:void(0)" class="btn btn-info btn-sm view_supplier" data-id="<?= $row['supplier_id'] ?>"><i class="fa fa-th-list"></i></a>
                            <a href="javascript:void(0)" class="btn btn-primary btn-sm edit_supplier" data-id="<?= $row['supplier_id'] ?>" data-name="<?= $row['name'] ?>"><i class="fa fa-edit"></i></a>
                            <a href="javascript:void(0)" class="btn btn-danger btn-sm delete_supplier" data-id="<?= $row['supplier_id'] ?>" data-name="<?= $row['name'] ?>"><i class="fa fa-trash"></i></a>
                        </td>
                    </tr>
                    <?php endwhile; ?>
...
$(function(){
    ...
    function paginateTable() {
        let rows = filteredRows.length ? filteredRows : $('#supplier_table tbody tr');
        let totalRows = rows.length;
        let totalPages = Math.ceil(totalRows / rowsPerPage);

        $('#supplier_table tbody tr').hide(); // hide all first
    ...
    function filterSupplierTable() {
        ...
        filteredRows = $('#supplier_table tbody tr').filter(function () {
